...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,align=center,node distance=2cm,
  thick,main node/.style={fill=blue!20,rectangle,draw,font=\small\bfseries}]
...
the specification language, although it has to be declarative and the
structure of the templates is similar to the structure of the typing
rules of the specification language.
...
\item[Judgments] correspond to the user defined judgments of the
  ...
  context. A valid context can be obtained by applying the context
  modifications from right to left to a context instance. A detail
  explanation of the semantics in our type checker will follow in
...
\item[Context lookups] consist of the context number
  ...
  \refcallout{lookup:errors}. We do not treat lookups as normal
...
Example~\ref{ex:premise-dependencies} would look like
\code|(1,[TAll(Var("X0"), Var("X1"))])|. Here \code|Var("X0")| and
\code|Var("X1")| are the generated variable names for \code|%x| and
\code|~T|. In this example \code|TAll(Var("X0"), Var("X1"))| is the
only output provided by the premise and \code|1| refers to the second
premise, because we sort premises of templates topological according
to the dependencies, as we will describe in the next section.
...
template. Note that in the example we hide the underscores in the
constructor names of the specification language. Due to that we see in
Figure~\ref{fig:template-example} the term \code|Var(Var("X52"))|
where actually the outermost \code|Var| constructor is part of the
target language and the inner \code|Var| is part of the specification
language.
...
which models type substitution in the SystemF, see
...
those error messages with the corresponding explicit equalities. Then
...
templates. The idea is that templates contained in \code|Fork|s are all
...
that each group contains only which-ambiguous templates. Two templates
are which-ambiguous if they have the same judgment number (i.e.\ the
position in the judgment declaration section) and if the term pattern
and context pattern of the conclusion are equal modulo variables. Of
the resulting template groups we wrap all groups with more than one
...
\code|Refl|. That unfolding creates from non-syntax directed templates
a set of syntax directed templates. Of course the unfolding can create
new ambiguities and not all templates are applicable at all. In the
current optimization step we will try to eliminate the newly created
which ambiguities. We do this in the same way as described before,
except that we now also try to proof the formula from
Definition~\ref{def:subsumes} by structural induction. Before we
...
that the following two rules have the same conclusion modulo variable
renaming.
...
Note that we can apply rule \code|S-arrow| as we want to retain this
...
After unfolding the non-syntax directed templates we collect
...
non-terminal productions are replaced by fresh and arbitrary
...
case, by substituting constants into the proposition we then to prove
...
judgment. This information are available as we read the program
...
If the selected rule is a fork, the evaluation works as follows. In
correspondence to the bindings in the conclusions context pattern, we
pop all terms from the contexts and bring them into scope. This is
needed for example in the typing rules for the freshness condition in
SystemF, see Appendix~\ref{appendix:systemf}. 
