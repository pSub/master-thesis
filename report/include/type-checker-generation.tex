...
We know that the subtyping relation \verb|<:| is reflexive by the rule
...
computed outputs and the generated constraints.
...
judgment number. Now the templates are searched for a template that
has the correct judgment identifier in its conclusion, matching input
positions and a context description that reflects the current state in
the store. We take the first template that matches as we have ensured
in Section~\ref{sec:constr-templ-optim} that there is only one such
template. If no template matches those criteria, we return the
constraint that always fails with an appropriate error message. In
general \verb|generate| returns a pair of the output and a constraint
set, where the output is optional. \todo{It is not so clear where the
  output comes from}
