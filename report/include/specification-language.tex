...
meta-variable of this class has the prefix \code|"~"|. Productions
that produce meta-variables of this class are added to the
non-terminals \code|Type| and \code|Exp|.
...
In Example~\ref{ex:rules} the rules \code|Subst-Eq| and \code|T-Tapp|
from the SystemF \todo{Reference} implementation are
shown. \code|Subst-Eq| has no premisses and an implicit equality in
the conclusion. Rule \code|T-Tapp| show how the expected output is
...
the type of the faculty function apply to three is \code|int| and the
second conjecture asserts that \code|fun x : int (x)| does not have
type \code|int|.
...
was define in a specification and the main module \code|SLTC| that
combines the previous modules.

The module \code|Common| is not particularly interesting therefore we
will not explain it in detail but mention it when it is used in other
modules.

The target language independent syntax of the specification language
is defined in the module \code|BaseLanguage|, which is parameterized
by the non-terminals \code|TypingJudgment| and
\code|MetaVariable|. Those non-terminals depend on the actual target
language and are therefore defined in \code|Generated|.

In a fresh project the module \code|Generated| does not exist in the
...
the syntax folder. This module contains empty productions for the
non-terminals \code|TypingJudgment| and \code|MetaVariable| and
ensures that a compilation of the whole project is possible, without a
specification (e.g.\ to run tests). The strategy \code|toSdf|
generates a specification specific \code|Generated| module.
...
\code|Generated| also contains context-free grammars for the context,
meta-variables and judgment declarations.

The strategy \code|make-contexts| generates for each context
declaration productions for the empty context, context bindings and
context look up. Figure~\ref{fig:context-productions} shows the
...
\textit{Name} is the name of the context declaration and \code|n| the
position of the context declaration.
...
second enables the use of meta-variables in error messages. The
\code|m| in the constructor of meta-variables is either the context
number in the case \textit{Scope} is a context and otherwise the
meta-variable class.
...
declarations. We generate for each judgment one production that
consists of all non-terminals separated by the separators from the
declaration. The constructor is in case of a built-in judgment the
name of the built-in, otherwise it is the judgment position append to
the string \code|TypingJudgment|.
